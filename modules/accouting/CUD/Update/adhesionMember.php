...
if($mark == $controle_POST) {
    ...
    if(date('n')>1&&date('n')<6) {
        $_POST['objet'] = 'Cotisation pour année '.($actualYear-1).' à septembre '.$actualYear.' de '.$infosMember[1]['prenom'].' '.$infosMember[1]['nom'].' numéro adhérant: '.$infosMember[0]['MemberNumber']; 
    } else {
      $_POST['objet'] = 'Cotisation pour année '.($actualYear).' à septembre '.($actualYear + 1).' de '.$infosMember[1]['prenom'].' '.$infosMember[1]['nom'].' numéro adhérant: '.$infosMember[0]['MemberNumber']; 
    }
    $member->recordCotisation (filter($_POST[$arrayKeys[0]]));
    array_shift($_POST);
